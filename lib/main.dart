...
class _MyHomePageState extends State<MyHomePage> {
  ...
  String _avatarImage = 'assets/images/image1.jpg';

  Future<GenderInfo?> _getGenderInfo(String name) async {
    const String baseUr1 = "http://api.genderize.io/";
    try {
      final response = await Dio().get(baseUr1, queryParameters: {"name": name});
    ...
    if (name.isNotEmpty) {
      ...
      setState(() {
        ...
        if (_genderInfo!.gender == 'male') {
          _avatarImage = 'assets/images/image2.jpg';
        } else if (_genderInfo!.gender == 'female') {
          _avatarImage = 'assets/images/image3.jpg';
        } else {
          _avatarImage = 'assets/images/image1.jpg';
        }
      });
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Gender Indetification'),
      ),
      body: Padding(
